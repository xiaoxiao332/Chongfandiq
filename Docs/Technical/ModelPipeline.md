...
`ArtSource/LastLight/build_m2_assets.py` 是独立后台 Blender 入口，生成 `M2Assets.blend` 与 `Assets/LastLight/Models/M2/` 中 14 个 FBX：种植箱、保护架、桥梁完好／损坏、温室、三种作物各三个阶段。不会运行 M1 生成循环，也不覆盖 SnowVillage。复用低多边形纯色材质，Unity 材质和 Prefab 由 M2ChapterBuilder 组装。

运行：`D:/BaiduSyncdisk/blender/blender.exe --background --factory-startup --python-exit-code 1 --python ArtSource/LastLight/build_m2_assets.py`。

内嵌字体来自 Noto CJK 官方仓库（SIL Open Font License）：https://github.com/notofonts/noto-cjk/tree/main/Sans 。字体和许可文件一同保存在 `Assets/LastLight/Fonts/`；普通玩家无需安装系统字体。
