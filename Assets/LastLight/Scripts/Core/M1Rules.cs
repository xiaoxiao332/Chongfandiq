...
namespace LastLight
{
    ...
    public enum Structure { Floor, Wall, Door, Window, Stove, Bed, Workbench, Storage, Barricade, Planter, Charger, Decoy, WatchLight, Table, Chair, Shelf, Lamp }
    ...
    public static class Catalog
    {
        ...
        public static Cost[] Recipe(Structure s)
        {
            switch(s){
                ...
                case Structure.Charger:return new[]{new Cost(Resource.Scrap,5),new Cost(Resource.Parts,2)};
                case Structure.Decoy:return new[]{new Cost(Resource.Wood,2),new Cost(Resource.Scrap,2)};
                case Structure.WatchLight:return new[]{new Cost(Resource.Scrap,3),new Cost(Resource.Parts,1)};
                case Structure.Lamp:return new[]{new Cost(Resource.Scrap,1),new Cost(Resource.Fiber,1)};
    ...
    public sealed class BuildGrid
    {
        ...
        public string Validate(Structure t,int x,int z,int dir)
        {
            ...
            if(t!=Structure.Barricade&&t!=Structure.Planter&&t!=Structure.Decoy&&t!=Structure.WatchLight&&!IsFloor(x,z))return L.K("t2ec8a9a3d5");
