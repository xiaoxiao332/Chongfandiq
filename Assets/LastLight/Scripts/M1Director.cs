...
namespace LastLight
{
    public sealed partial class M1Director : MonoBehaviour
    {
        ...
        {
            await Global.BeginSessionAsync(s=>{s.Register(new M1EconomySystem());s.Register(new M1ConstructionSystem());s.Register(new M1SurvivalSystem());s.Register(new M1StorySystem());s.Register(new M1ThreatSystem());if(m2)s.Register(new M2WorldSystem());if(IsM3)s.Register(new M3WorldSystem());});
        ...
        public async Task TravelAsync(string destination)
        {
            ...
            try
            {
                ...
                var target=destination=="Relay"&&IsM3?relay:destination=="Home"?home:destination=="Workshop"?workshop:destination=="Greenhouse"&&m2?greenhouse:throw new ArgumentException("Unknown destination: "+destination);
