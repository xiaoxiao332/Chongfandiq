...
namespace LastLight
{
    public sealed partial class M1Beast:MonoBehaviour
    {
        ...
        public void Initialize(M1Session s,bool attackHome,Transform model){session=s;raid=attackHome;origin=transform.position;visual=model;health=80;timer=0;state=0;hitIds.Clear();navigation=GetComponent<M1Navigator>()??gameObject.AddComponent<M1Navigator>();navigation.Initialize(s,1.5f);
            if(warning==null){var line=new GameObject(L.K("t7095f76453"));line.transform.SetParent(transform,false);warning=line.AddComponent<LineRenderer>();}warning.sharedMaterial=s.SignalMaterial;warning.positionCount=5;warning.widthMultiplier=.06f;warning.loop=false;warning.enabled=false;}
        public void Hit(float damage){if(health<=0)return;health-=damage;session.Sound(2);if(health<=0){session.Notify(raid?L.K("tf26c241833"):L.K("t6c22f8cbfd"));warning.enabled=false;gameObject.SetActive(false);}}
        private void Update(){if(session==null||!Active)return;if(session.Paused){navigation?.Stop();return;}float dt=Time.deltaTime;if(kind!=M3EnemyKind.Beast){TickM3Enemy(dt);return;}timer-=dt;
            ...
            if(state==0){Move(target,dt);if(distance<7&&timer<=0){state=1;navigation.Stop();timer=1.25f;direction=(target-p).normalized;transform.rotation=Quaternion.LookRotation(direction);warning.enabled=true;Vector3 side=Vector3.Cross(Vector3.up,direction)*.65f;Vector3 start=p+Vector3.up*.15f;warning.SetPositions(new[]{start-side,start+direction*6-side,start+direction*6+side,start+side,start-side});session.Sound(3);}}
                ...
                if(raid&&Vector3.Distance(transform.position,session.RaidTarget)<1.6f&&hitIds.Add(-1)){session.RecordRaidStrike();session.Notify(L.K("t124b9dbf18")+session.RaidStrikes+L.K("t9c24052e71"));}
