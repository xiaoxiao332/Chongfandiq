...
namespace LastLight.Editor
{
    ...
    {
        public const string Root="Assets/LastLight/M3Generated";
        public const string Boot="Assets/LastLight/Scenes/M3Journey.unity";
        ...
        [MenuItem("Tools/LastLight/M3/Generate Prototype")]
        public static void Generate()
        {
            ...
            group=Settings.FindGroup("LastLight M3 Local")??Settings.CreateGroup("LastLight M3 Local",false,false,true,null,typeof(BundledAssetGroupSchema),typeof(ContentUpdateGroupSchema));
            ...
            try
            {
                ...
                foreach(Structure kind in Enum.GetValues(typeof(Structure)).Cast<Structure>().Take(17))MakeStructure(kind);
                ...
                MakeScene(true);MakeScene(false);MakeScene(false,true);MakeScene(false,false,true);MakeBoot(global);
        ...
        {
            var prefab=AssetDatabase.LoadAssetAtPath<GameObject>((System.IO.File.Exists("Assets/LastLight/Models/M3/"+name+".fbx")?"Assets/LastLight/Models/M3/":System.IO.File.Exists("Assets/LastLight/Models/M2/"+name+".fbx")?"Assets/LastLight/Models/M2/":"Assets/LastLight/Models/")+name+".fbx");if(prefab==null)throw new InvalidOperationException(L.K("t75c7c720c2")+name);
        ...
        private static void SavePrefab(GameObject go,string name){string path=Root+"/Prefabs/"+name+".prefab";PrefabUtility.SaveAsPrefabAsset(go,path);Address(path,"LastLight/M3/Prefabs/"+name);Object.DestroyImmediate(go);}
        ...
        private static PanelDefinition Definition(string id,UILayer layer,bool modal,bool pause,bool closable)=>new PanelDefinition{Id=id,Address="LastLight/M3/UI/"+id,Layer=layer,Modal=modal,Pause=pause,Closable=closable,Lifetime=SystemLifetime.Session};
        ...
        private static void MakePanel(PanelDefinition definition)
        {
            ...
            if(!hud&&!loading)for(int i=0;i<(build?21:15);i++)
            {
                int columns=3;float width=build?150:365;float y=build?100:-105;var buttonRect=Rect("Action "+i,card);Box(buttonRect,new Vector2(width,build?56:67),new Vector2((i%columns-(columns-1)*.5f)*(width+16),y-(i/columns)*(build?61:77)));
                Fill(buttonRect,new Color(.12f,.24f,.27f),true);var button=buttonRect.gameObject.AddComponent<Button>();var label=Text(buttonRect,"Label","",21,new Vector2(width-18,build?52:63),Vector2.zero);label.alignment=TextAnchor.MiddleCenter;label.resizeTextForBestFit=true;label.resizeTextMinSize=16;label.resizeTextMaxSize=24;buttons.Add(button);
            }
        ...
        private static void MakeBoot(GlobalManager root)
        {
            ...
            try{new GameObject("M1 Director").AddComponent<M1Director>().ConfigureM3(root);new GameObject("Bootstrap Camera").AddComponent<Camera>().enabled=false;var light=new GameObject("Bootstrap Light").AddComponent<Light>();light.type=LightType.Directional;light.enabled=false;EditorSceneManager.SaveScene(scene,Boot);}
        ...
        private static void MakeScene(bool home,bool greenhouse=false,bool relay=false)
        {
            string id=home?"Home":relay?"Relay":greenhouse?"Greenhouse":"Workshop";var scene=EditorSceneManager.NewScene(NewSceneSetup.EmptyScene,NewSceneMode.Additive);SceneManager.SetActiveScene(scene);
            try
            {
                ...
                AddM2Environment(scene,home,greenhouse,nodes);AddM3Environment(scene,home,greenhouse,relay,nodes,beasts,navigationRoot.transform);
                ...
                string path=Root+"/Scenes/"+id+".unity";EditorSceneManager.SaveScene(scene,path);Address(path,"LastLight/M3/Scenes/"+id);
            }
            finally{EditorSceneManager.CloseScene(scene,true);}
        }
        [MenuItem("Tools/LastLight/M3/Open Prototype")]
        ...
        [MenuItem("Tools/LastLight/M3/Validate Configuration")]
