...
namespace LastLight.Editor
{
    public static partial class M2ChapterBuilder
    {
        ...
        [MenuItem("Tools/LastLight/M2/Build Windows")]
        public static void BuildWindows()
        {
            ...
            Directory.CreateDirectory("Builds/LastLight/M2");Directory.CreateDirectory("Captures/LastLight/M2");
            ...
            try
            {
                ...
                var result=BuildPipeline.BuildPlayer(new BuildPlayerOptions{scenes=new[]{Boot},locationPathName="Builds/LastLight/M2/LastLight.exe",target=BuildTarget.StandaloneWindows64,options=BuildOptions.None});
                File.WriteAllText("Captures/LastLight/M2/build-result.json",JsonUtility.ToJson(new BuildEvidence{result=result.summary.result.ToString(),errors=(int)result.summary.totalErrors,warnings=(int)result.summary.totalWarnings,size=result.summary.totalSize.ToString(),utc=DateTime.UtcNow.ToString("O")}));
                ...
                Debug.Log("M2 Windows build succeeded: Builds/LastLight/M2/LastLight.exe");
            }
            finally
            {
