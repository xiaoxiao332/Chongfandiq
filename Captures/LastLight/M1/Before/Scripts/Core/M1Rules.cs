...
namespace LastLight
{
    ...
    public sealed class BuildGrid
    {
        ...
        public string Validate(Structure t,int x,int z,int dir)
        {
            ...
            if(Catalog.Edge(t)){
                ...
                Items.Remove(candidate);return invalid?"封闭房间必须留门；请将一面墙换成门":"";
            }
        ...
        public HashSet<(int,int)> Room(int x,int z,out bool door)
        {
            ...
            while(q.Count>0){var c=q.Dequeue();if(!visited.Add(c))continue;if(visited.Count>225){open=true;break;}
                for(int d=0;d<4;d++){var e=EdgeAt(c.Item1,c.Item2,d);if(e!=null){door|=e.type==Structure.Door;continue;}
        ...
        private bool AllFacilitiesReachable()
        {
            ...
            while(q.Count>0){var c=q.Dequeue();if(!visited.Add(c))continue;
                for(int d=0;d<4;d++){int x=c.Item1+DX[d],z=c.Item2+DZ[d];if(x< -8||x>8||z< -8||z>8||Occupied(x,z)||visited.Contains((x,z)))continue;
                    var e=EdgeAt(c.Item1,c.Item2,d);if(e!=null&&e.type!=Structure.Door)continue;q.Enqueue((x,z));}}
            return Items.Where(b=>b.type!=Structure.Floor&&!Catalog.Edge(b.type)).All(b=>Enumerable.Range(0,4).Any(d=>visited.Contains((b.x+DX[d],b.z+DZ[d]))));
        }
    }
    public sealed class StoryState
    {
        ...
        public bool Choose(int option,Inventory storage)
        {
            ...
            choice=option;promise=option==2?1:0;shortcut=option==1;return true;
        }
