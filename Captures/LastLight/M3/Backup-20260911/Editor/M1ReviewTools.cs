...
{
    public static class M1ReviewTools
    {
        ...
        private static async Task CaptureMatrix()
        {
            ...
            string folder="Captures/LastLight/M1/Layout-"+DateTime.Now.ToString("yyyyMMdd-HHmmss");Directory.CreateDirectory(folder);var report=new StringBuilder();
            try
            {
                ...
                {
                    int index=(int)groupType.GetMethod("GetTotalCount").Invoke(group,null);int custom=(int)groupType.GetMethod("GetCustomCount").Invoke(group,null);
                    var value=Activator.CreateInstance(sizeType,new object[]{mode,size.x,size.y,"M1 review temporary"});groupType.GetMethod("AddCustomSize").Invoke(group,new[]{value});selected.SetValue(view,index);view.Repaint();await Frames(8);
                    try
                    {
                        foreach(string state in new[]{"HUD","Inventory","Build","Journal"})
                        {
                            d.Current.CloseWindow();await Frames();
                            if(state=="Inventory")d.Current.OpenInventory();if(state=="Build")d.Current.ToggleBuild();if(state=="Journal")d.Current.OpenJournal();
                            ...
                            string path=folder+"/"+state+"-"+size.x+"x"+size.y+".png";ScreenCapture.CaptureScreenshot(path);await Frames(8);double deadline=Time.realtimeSinceStartupAsDouble+10;while(!File.Exists(path)&&Time.realtimeSinceStartupAsDouble<deadline)await Task.Yield();if(!File.Exists(path))throw new IOException("截图未落盘 "+path);
                            report.AppendLine("PASS "+state+" "+size.x+"x"+size.y+" : content bounds, button text, actual screenshot");Status=state+" "+size;
                        }
                    }
                    finally
                    {
                ...
                Status="PASS: 16 actual Game View screenshots";
            }
            catch(Exception ex){Status="FAIL: "+ex.Message;report.AppendLine(ex.ToString());Debug.LogException(ex);}
            finally{d.Current.CloseWindow();selected.SetValue(view,previous);view.Repaint();File.WriteAllText(folder+"/Validation.txt",report.ToString());}
        }
    }
}
