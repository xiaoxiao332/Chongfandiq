...
{
    public static class M1PrototypeBuilder
    {
        public const string Root="Assets/LastLight/M1Generated";
        public const string Boot="Assets/LastLight/Scenes/M1Prototype.unity";
        ...
        [MenuItem("Tools/LastLight/M1/Generate Prototype")]
        public static void Generate()
        {
            if(EditorApplication.isPlayingOrWillChangePlaymode)throw new InvalidOperationException("请先退出 Play Mode。");
            for(int i=0;i<SceneManager.sceneCount;i++)if(SceneManager.GetSceneAt(i).isDirty)throw new InvalidOperationException("当前场景有未保存修改，生成器停止。");
            ...
            group=Settings.FindGroup("LastLight M1 Local")??Settings.CreateGroup("LastLight M1 Local",false,false,true,null,typeof(BundledAssetGroupSchema),typeof(ContentUpdateGroupSchema));
            ...
            try
            {
                ...
                foreach(Structure kind in Enum.GetValues(typeof(Structure)).Cast<Structure>().Take(9))MakeStructure(kind);
                ...
                MakeScene(true);MakeScene(false);MakeBoot(global);
        ...
        {
            var prefab=AssetDatabase.LoadAssetAtPath<GameObject>("Assets/LastLight/Models/"+name+".fbx");if(prefab==null)throw new InvalidOperationException("缺少模型："+name);
        ...
        {
            var go=new GameObject(kind.ToString());Model(kind.ToString(),go.transform);go.AddComponent<M1Built>();
        ...
        private static void SavePrefab(GameObject go,string name){string path=Root+"/Prefabs/"+name+".prefab";PrefabUtility.SaveAsPrefabAsset(go,path);Address(path,"LastLight/M1/Prefabs/"+name);Object.DestroyImmediate(go);}
        ...
        private static PanelDefinition Definition(string id,UILayer layer,bool modal,bool pause,bool closable)=>new PanelDefinition{Id=id,Address="LastLight/M1/UI/"+id,Layer=layer,Modal=modal,Pause=pause,Closable=closable,Lifetime=SystemLifetime.Session};
        ...
        private static void MakePanel(PanelDefinition definition)
        {
            ...
            var card=Rect("Content",rect);Vector2 dimensions=hud?new Vector2(470,490):build?new Vector2(490,880):new Vector2(1120,870);Box(card,dimensions,Vector2.zero);
            ...
            float bodyHeight=hud?385:build?220:490;
            ...
            if(!hud&&!loading)for(int i=0;i<12;i++)
            {
                int columns=build?2:3;float width=build?207:335;float y=build?65:-190;var buttonRect=Rect("Action "+i,card);Box(buttonRect,new Vector2(width,57),new Vector2((i%columns-(columns-1)*.5f)*(width+16),y-(i/columns)*67));
                Fill(buttonRect,new Color(.12f,.24f,.27f),true);var button=buttonRect.gameObject.AddComponent<Button>();var label=Text(buttonRect,"Label","",21,new Vector2(width-18,51),Vector2.zero);label.alignment=TextAnchor.MiddleCenter;buttons.Add(button);
            }
            rect.gameObject.AddComponent<M1Panel>().Configure(title,body,buttons.ToArray());string path=Root+"/Prefabs/"+definition.Id+".prefab";PrefabUtility.SaveAsPrefabAsset(rect.gameObject,path);Address(path,definition.Address);Object.DestroyImmediate(rect.gameObject);
        }
        ...
        private static void MakeBoot(GlobalManager root)
        {
            ...
            try{new GameObject("M1 Director").AddComponent<M1Director>().Configure(root);new GameObject("Bootstrap Camera").AddComponent<Camera>().enabled=false;var light=new GameObject("Bootstrap Light").AddComponent<Light>();light.type=LightType.Directional;light.enabled=false;EditorSceneManager.SaveScene(scene,Boot);}
        ...
        private static void MakeScene(bool home)
        {
            string id=home?"Home":"Workshop";var scene=EditorSceneManager.NewScene(NewSceneSetup.EmptyScene,NewSceneMode.Additive);SceneManager.SetActiveScene(scene);
            try
            {
                ...
                if(home)
                {
                    Node(nodes,"home-terminal","Terminal",new Vector3(0,0,-10),NodeKind.Terminal,"家园终端：目标 / 无线电 / 防御");
                    Node(nodes,"emergency-depot","Storage",new Vector3(-3,0,-16),NodeKind.Depot,"公共仓储：存入随身物资");
                    Node(nodes,"emergency-bench","Workbench",new Vector3(3,0,-16),NodeKind.EmergencyBench,"应急手工台：燃料 / 零件");
                    var heat=Node(nodes,"emergency-stove","Stove",new Vector3(0,0,-7),NodeKind.EmergencyStove,"应急暖炉：修复 / 添燃料");stove=new GameObject("Warm light").AddComponent<Light>();stove.transform.SetParent(heat.transform,false);stove.transform.localPosition=new Vector3(0,1.5f,0);stove.type=LightType.Point;stove.color=new Color(1,.55f,.19f);stove.range=10;stove.intensity=4;stove.enabled=false;
                    Node(nodes,"maintenance-pod","Pod",new Vector3(-6,0,-16),NodeKind.Pod,"维护舱：基本休息");
                    var worker=Node(nodes,"liang","Liang",new Vector3(-4,0,-14),NodeKind.Liang,"梁：启动 / 建筑修复岗位");liang=worker.transform;
                    Node(nodes,"workshop-sign","Terminal",new Vector3(20,0,-10),NodeKind.Travel,"沿旧路前往旧工坊");Node(nodes,"aid-drain","Scrap",new Vector3(21,0,12),NodeKind.AidRoute,"岑标记的旧排水道（援助后开放）");
                    for(int i=0;i<8;i++)Node(nodes,"wood-"+i,"Wood",new Vector3(i<4?-19:19,0,-3+(i%4)*5),NodeKind.Pickup,"拾取枯木 ×4",Resource.Wood,4);
                    for(int i=0;i<3;i++){Node(nodes,"stone-"+i,"Stone",new Vector3(-10+i*5,0,19),NodeKind.Pickup,"拾取碎石 ×3",Resource.Stone,3);Node(nodes,"fiber-"+i,"Fiber",new Vector3(-19,0,-13-i*3),NodeKind.Pickup,"拾取干草 ×3",Resource.Fiber,3);Node(nodes,"food-"+i,"Ration",new Vector3(10+i*3,0,-20),NodeKind.Pickup,"采集口粮 ×2",Resource.Ration,2);Node(nodes,"scrap-"+i,"Scrap",new Vector3(20,0,-20+i*3),NodeKind.Pickup,"徒手回收废金属 ×3",Resource.Scrap,3);}
                ...
                {
                    Node(nodes,"return-road","Terminal",new Vector3(0,0,-35),NodeKind.Travel,"沿旧路返回家园");
                    Node(nodes,"workshop-core","Core",new Vector3(7,0,47),NodeKind.Core,"取回梁的启动件");
                    Node(nodes,"departure-record","Terminal",new Vector3(-8,0,22),NodeKind.Record,"读取撤离记录");
                    Node(nodes,"return-shortcut","Terminal",new Vector3(12,0,45),NodeKind.Shortcut,"打开回家捷径（取件后可用）");
                    for(int i=0;i<8;i++)Node(nodes,"salvage-"+i,i%3==0?"Wood":"Scrap",new Vector3(i%2==0?-15:14,0,-17+i*8),NodeKind.Pickup,i%3==0?"回收木材 ×4":"回收废金属 ×4",i%3==0?Resource.Wood:Resource.Scrap,4);
                ...
                string path=Root+"/Scenes/"+id+".unity";EditorSceneManager.SaveScene(scene,path);Address(path,"LastLight/M1/Scenes/"+id);
            }
            finally{EditorSceneManager.CloseScene(scene,true);}
        }
        [MenuItem("Tools/LastLight/M1/Open Prototype")]
        ...
        [MenuItem("Tools/LastLight/M1/Validate Configuration")]
        public static void Validate()
        {
            var catalog=AssetDatabase.LoadAssetAtPath<PanelCatalog>(Root+"/Configuration/Panels.asset");if(catalog==null)throw new InvalidOperationException("先生成原型。");
            foreach(var panel in catalog.Panels){var prefab=AssetDatabase.LoadAssetAtPath<GameObject>(Root+"/Prefabs/"+panel.Id+".prefab");if(prefab==null||prefab.GetComponent<M1Panel>()==null)throw new InvalidOperationException("无效面板 "+panel.Id);var rect=prefab.GetComponent<RectTransform>();if(rect.anchorMin!=Vector2.zero||rect.anchorMax!=Vector2.one||rect.offsetMin!=Vector2.zero||rect.offsetMax!=Vector2.zero)throw new InvalidOperationException("面板根布局错误 "+panel.Id);}
            Debug.Log("M1 配置检查：九个面板、独立场景和专用资源目录已生成。");
        }
