...
namespace LastLight
{
    public sealed partial class M1Director
    {
        ...
        public string Prefix=>m2?"LastLight/M2/":"LastLight/M1/";
        ...
        {
            Saves=new M2SaveStore(SaveDirectoryOverride??System.IO.Path.Combine(Application.persistentDataPath,"M2Saves"));
            M2Settings.Load();
        ...
        {
            Economy.Restore(data);Construction.Restore(data);Survival.Restore(data);Narrative.Restore(data);Threat.Restore(data);World.Restore(data.world);
        ...
        public async Task LoadSlotAsync(int slot)
        {
            ...
            try{data=Saves.Load(slot,out backup);}catch(Exception ex){SaveMessage=L.K("t63370a8fa5")+ex.Message;Current?.Notify(SaveMessage);return;}
