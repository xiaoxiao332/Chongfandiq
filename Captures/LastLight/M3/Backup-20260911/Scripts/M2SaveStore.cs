...
namespace LastLight
{
    ...
    [Serializable] public sealed class M2SaveData
    {
        ...
        {
            if(version!=1)throw new InvalidDataException("Unsupported save version: "+version);
            if(scene!="Home"&&scene!="Workshop"&&scene!="Greenhouse")throw new InvalidDataException("Unknown scene");
    ...
    public sealed class M2SaveStore
    {
        ...
        public M2SaveStore(string directory){DirectoryPath=directory;}
        public string PathFor(int slot){if(slot<0||slot>3)throw new ArgumentOutOfRangeException(nameof(slot));return Path.Combine(DirectoryPath,slot==0?"auto.json":"slot"+slot+".json");}
