...
namespace LastLight
{
    ...
    public sealed partial class M1Actor:MonoBehaviour
    {
        ...
        public void Initialize(M1Session owner){session=owner;if(actions==null){if(inputs==null||animator==null||view==null)throw new System.InvalidOperationException(name+L.K("td860e3479c"));actions=Instantiate(inputs);move=actions.FindAction("Player/Move",true);sprint=actions.FindAction("Player/Sprint",true);attack=actions.FindAction("Player/Attack",true);interact=actions.FindAction("Player/Interact",true);foreach(var t in animator.GetComponentsInChildren<Transform>())if(t.name=="UpperArm.R")arm=t;}if(owner is M1GameSession gameSession&&gameSession.M2Enabled)ConfigureM2Input();actions.FindActionMap("Player").Enable();}
        ...
        private void OnDestroy(){CancelRebind();if(actions!=null)Destroy(actions);}
        private void Update()
        {
            ...
            if(session is M1GameSession m2s&&m2s.M2Enabled?Pressed("Eat"):k!=null&&k.fKey.wasPressedThisFrame)session.Eat();
            if(interact.WasPressedThisFrame()|| (!(session is M1GameSession ms&&ms.M2Enabled)&&k!=null&&k.eKey.wasPressedThisFrame))session.InteractNearest();
        ...
        private void LateUpdate(){WorkPose();if(session!=null&&!session.Paused&&arm!=null&&attackTimer>0)arm.localRotation*=Quaternion.Euler(-100*Mathf.Sin((.75f-attackTimer)/.75f*Mathf.PI),0,0);}
