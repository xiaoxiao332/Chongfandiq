...
M2 入口为 `Assets/LastLight/Scenes/M2Chapter.unity`，内容在 `Assets/LastLight/M2Generated/`，Addressables 分组为 `LastLight M2 Local`，地址前缀 `LastLight/M2/`。`M1Director` 的序列化 M2 标记选择入口配置，复用既有经济、建造、生存、叙事、威胁系统；新增 Session 级 `M2WorldSystem`。M1 入口继续使用原有目录和配置。

- `M2SaveStore` 保存带版本与 SHA-256 校验和的 JSON 信封。写临时文件、刷新磁盘、回读校验，再替换正式文件；损坏主档不覆盖有效备份，原损坏内容另存。读取主档失败尝试备份，均失败报告并保留文件。
- `M2Snapshots` 为现有系统提供 Capture/Restore。库存、建筑编号、床位、采集冷却、剧情选择和日志文本记录均属于会话；作物按建筑编号保存。场景敌人保留位置、生命、攻击阶段、计时和本次命中集合。
- 读取先验证全部数据，再停止场景输入和异步建筑加载，恢复系统与对应场景；失败尝试恢复读取前快照。场景退出通过 QuiesceAsync 等待已取消的建筑任务，不把取消当作成功加载。
- 主菜单启动不自动保存。关键菜单命令请求自动保存，待事务与场景适配完成后提交；旅行、睡眠和桥梁完工同样触发。普通手工维修进度在读档时取消，材料只在完成时扣除，因此不会部分扣费。
- `M2WorldSystem` 统一农业成长、春季计日、桥梁进度。睡眠使用与主动时间相同的生存与农业规则分步结算，不允许跳过已知威胁；春季警报在返家后开始准备窗口。
- `L.K/L.F` 构造稳定文本记录，`L.Resolve` 在显示边界选择语言。日志保存文本键及参数，不保存已翻译的句子。`LocalizationTables.cs` 对应 `ArtSource/LastLight/localization-source.json` 与 `en.tsv`。
- M2 面板序列化引用内嵌 Noto Sans CJK SC 字体；许可在 `Assets/LastLight/Fonts/LICENSE.txt`。字体来源：https://github.com/notofonts/noto-cjk 。M1 保留其原字体路径。
- 模型生成入口 `ArtSource/LastLight/build_m2_assets.py` 只覆盖 `Assets/LastLight/Models/M2/` 中 14 个负责的 FBX 和 `M2Assets.blend`。M2ChapterBuilder 只生成 M2Generated、M2Chapter 场景和 M2 分组；BuildWindows 明确配置 M2 启动场景并生成本地 Addressables。
- Windows 输出为 `Builds/LastLight/M2/LastLight.exe`。`--m2-probe <目录>` 是显式独立构建诊断，使用隔离存档并写报告；附加 `--m2-probe-load` 在新进程中验证持久化。普通启动不会运行诊断。
